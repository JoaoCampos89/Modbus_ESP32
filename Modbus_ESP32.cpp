...
Modbus_ESP32::Modbus_ESP32() {
    _regs_head = 0;
    _regs_last = 0;
}

TRegister* Modbus_ESP32::searchRegister(word address) {
...
void Modbus_ESP32::addReg(word address, word value) {
    TRegister *newreg;
...
bool Modbus_ESP32::Reg(word address, word value) {
    TRegister *reg;
...
word Modbus_ESP32::Reg(word address) {
    TRegister *reg;
    reg = this->searchRegister(address);
    if(reg)
        return(reg->value);
...
void Modbus_ESP32::addHreg(word offset, word value) {
    this->addReg(offset + 40001, value);
}

bool Modbus_ESP32::Hreg(word offset, word value) {
    return Reg(offset + 40001, value);
}

word Modbus_ESP32::Hreg(word offset) {
    return Reg(offset + 40001);
}

#ifndef USE_HOLDING_REGISTERS_ONLY
    void Modbus_ESP32::addCoil(word offset, bool value) {
        this->addReg(offset + 1, value?0xFF00:0x0000);
    }

    void Modbus_ESP32::addIsts(word offset, bool value) {
        this->addReg(offset + 10001, value?0xFF00:0x0000);
    }

    void Modbus_ESP32::addIreg(word offset, word value) {
        this->addReg(offset + 30001, value);
    }

    bool Modbus_ESP32::Coil(word offset, bool value) {
        return Reg(offset + 1, value?0xFF00:0x0000);
    }

    bool Modbus_ESP32::Ists(word offset, bool value) {
        return Reg(offset + 10001, value?0xFF00:0x0000);
    }

    bool Modbus_ESP32::Ireg(word offset, word value) {
        return Reg(offset + 30001, value);
    }

    bool Modbus_ESP32::Coil(word offset) {
        if (Reg(offset + 1) == 0xFF00) {
            return true;
        } else return false;
    }

    bool Modbus_ESP32::Ists(word offset) {
        if (Reg(offset + 10001) == 0xFF00) {
            return true;
        } else return false;
    }

    word Modbus_ESP32::Ireg(word offset) {
        return Reg(offset + 30001);
    }
#endif


void Modbus_ESP32::receivePDU(byte* frame) {
    byte fcode  = frame[0];
...
void Modbus_ESP32::exceptionResponse(byte fcode, byte excode) {
    //Clean frame buffer
    free(_frame);
    _len = 2;
...
void Modbus_ESP32::readRegisters(word startreg, word numregs) {
    //Check value (numregs)
...
void Modbus_ESP32::writeSingleRegister(word reg, word value) {
...
void Modbus_ESP32::writeMultipleRegisters(byte* frame,word startreg, word numoutputs, byte bytecount) {
    //Check value
...
#ifndef USE_HOLDING_REGISTERS_ONLY
void Modbus_ESP32::readCoils(word startreg, word numregs) {
    //Check value (numregs)
...
void Modbus_ESP32::readInputStatus(word startreg, word numregs) {
    //Check value (numregs)
    if (numregs < 0x0001 || numregs > 0x07D0) {
...
void Modbus_ESP32::readInputRegisters(word startreg, word numregs) {
    //Check value (numregs)
    if (numregs < 0x0001 || numregs > 0x007D) {
...
void Modbus_ESP32::writeSingleCoil(word reg, word status) {
    //Check value (status)
...
void Modbus_ESP32::writeMultipleCoils(byte* frame,word startreg, word numoutputs, byte bytecount) {
    //Check value
